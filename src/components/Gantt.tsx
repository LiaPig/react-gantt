...
declare interface GanttProps {
  ...
  ganttData: GanttDataItem<unknown>[]
  itemTitle?: string
  ...
  onActivityClick?: (
    ...
    ganttItem: GanttDataItem<unknown>,
    ganttIndex: number
  ) => void
...
const getActivityWidth = (startDate: string, endDate: string) => {
  const diffNum =
    dayjs(new Date(endDate)).diff(new Date(startDate), 'hour') / 24
  return `${(diffNum + 1) * activityWidth - 2 * activityPadding}px`
}
const getActivityLeft = (
  startDate: string,
  dateList: { yearMonthDate: string }[]
) => {
...
const getActivityTop = (
  startDate: string,
  endDate: string,
  beforeList: GanttActivity<unknown>[],
) => {
  const lastList = beforeList.filter(
    (item) =>
      isOverlap(item.startDate, item.endDate, startDate, endDate) &&
      item.startDate &&
      item.startDate !== '--' &&
      item.endDate &&
      item.endDate !== '--'
  ...
  // 前面还有空位
  const lastTopList = lastList.map(
    (item) => (Number(item.style?.top.split('px')?.[0]) - 2) / gridHeight
  )
  count = 0
...
const soryByTwoDate = (activityList: GanttActivity<unknown>[]) => {
...
function Gantt({
  ...
  const monthList = useMemo(() => {
    const tipTimeList =
      dateTooltipList?.map((item) => new Date(item.date).getTime()).reverse() ||
      []
    const dayList = ganttData
      .map((item) =>
        item?.activities?.map((item) => [item.startDate, item.endDate]).flat()
      )
      .flat()
    const dayTimeList = Array.from(
      new Set([
        ...tipTimeList,
        ...dayList
    ...
    return getMonthsInRange(minDay, maxDay).map((item) => {
      ...
      const datesOfMonth = getDatesOfMonth(item)?.filter(
        (item) => item >= minDay - 24 * 60 * 60 * 1000 && item <= maxDay
      )
      const weeksOfMonth = Object.entries(groupDatesByWeek(datesOfMonth)).map(
        ([value, list]) => ({
          value,
          data: list?.map((item: number) => {
            ...
            return {
              ...
              day: current.day(),
              dayStr: current.locale('zh-cn').format('dd'),
              isToday,
              isHoliday,
              yearMonthDate,
              style: {
  ...
  }, [dateTooltipList, ganttData, getDateBackground])
  const dateList = useMemo(() => {
    return monthList
      .map((item) => item?.weekList)
      .flat()
      .map((item) => item.data)
      .flat()
  }, [monthList])
  const logTimeoutRef = useRef<number>()
  const execTimesRef = useRef(0)
  const itemHeightList = useMemo(() => {
    execTimesRef.current++
    clearTimeout(logTimeoutRef.current)
    logTimeoutRef.current = setTimeout(() => {
      // console.log(execTimesRef.current)
      execTimesRef.current = 0
    }, 1000)
    return ganttData.map((ganttItem) => {
      const dateActivityNumList = dateList.map((item) =>
        getActivityCountInDay(item.yearMonthDate, ganttItem.activities)
      )
      ...
      const height =
        maxActivitiesNum <= 1
          ? activityWidth
          : `${maxActivitiesNum * gridHeight + 2 * activityPadding}px`
      return {
  ...
  const activityData = useMemo(() => {
    return ganttData?.map((item, index) => {
      ...
      const newActivities: GanttActivity<unknown>[] = []
  ...
  useEffect(() => {
    ...
    if (todayOffsetLeft > 0) {
      scrollXRef.current.scrollLeft =
        todayOffsetLeft - scrollXRef.current.clientWidth / 2
    } else {
  ...
  return (
    <div className="gantt relative max-w-full">
      <div className={`gantt-header w-full bg-white h-[${activityWidth}px]`}>
        {header}
      </div>
      <div
      ...
      >
        {ganttData?.length > 0 && (
          <>
            ...
            >
              ...
              >
                <div
                  ...
                  style={{
                    width: `${(activityWidth + activityPadding) * 4}px`,
                  }}
                ></div>
            ...
            <div
              ref={scrollXRef}
              className="gantt-inner relative overflow-x-auto"
            ...
            >
              <div
                ...
                style={{
                  ...
                  width: `${dateList?.length * activityWidth + itemWidth}px`,
                }}
              >
                <MonthList data={monthList} dateTooltipList={dateTooltipList} />
              </div>
              <div
                className="flex"
                style={{
              ...
              >
                ...
                <div
                  className="gantt-guide-content sticky left-0 text-center bg-[#fff] z-10"
