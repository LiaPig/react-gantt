      ...
      <Popover
        ...
        content={
          <Space direction="vertical" size={20}>
            {dateList?.map((item) => <div key={item.date}>{item.content}</div>)}
          </Space>
        }
      >
        <div className="relative">
          {children}
          <div className="absolute left-[10px] top-[-12px] w-[20px] h-[20px] text-[12px] text-white leading-[20px] whitespace-nowrap  text-center bg-[#ff4d4f] rounded-[10px]">
            {dateList?.length}
          </div>
        </div>
      </Popover>
...
const MonthList = memo(function ({ data, dateTooltipList }: Props) {
  return (
    <div className="gantt-inner-month-list flex bg-white">
      {data?.map((item) => (
        <div
          key={`${item?.year}-${item?.monthStr}`}
        ...
        >
          ...
          <div
            ...
            style={{
              height: `${activityWidth}px`,
              width:
                item.weekList?.length === 1 &&
                item.weekList[0].data.length === 1
                  ? `${activityWidth}px`
                  : 'auto',
            }}
          >
